...
export default function VoiceInterview({ config, onComplete }) {
  ...
  const characterName = isPostCounsellor || isWeekendPlan
    ...
    : isPostAdmission
      ? 'Sophie'
      : isDayTwoCheckin || isDayThreeFollowup || isPostCampPushback || isPostCampWrap || isScopeCall || isIdeaCheckin || isBuildKickoff || isNamingCall || isMarketingCall || isFrustratedCall || isTensraCall || isNameserverCall || isHostingUpdateCall || isInstallCall
        ? 'Coach Nova'
        : 'Scout'
  const sessionLabel = isInstallCall
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        const systemPrompt = isInstallCall
        ...
        const greetingMessage = isInstallCall
        ...
        await adapter.connect({
          ...
          systemPrompt,
          tools,
          voiceName: (isDayTwoCheckin || isDayThreeFollowup || isPostCampPushback || isPostCampWrap || isScopeCall || isIdeaCheckin || isBuildKickoff || isNamingCall || isMarketingCall || isFrustratedCall || isTensraCall || isNameserverCall || isHostingUpdateCall || isInstallCall) ? 'Charon' : 'Zephyr',
          language: 'en',
          greetingMessage,
        })
